"""
This module processes video frames and provides an ASCII output
"""
...
from . import render_strategies as re
from . import image_processing as ipe
...
class AsciiStrategy(re.RenderStrategy):
    ...
    def convert_frame_pixels_to_ascii(self, frame, dimensions=DEFAULT_TERMINAL_SIZE, new_line_chars=False):
        """
        Replace all pixels with colored characterss and return the resulting string.
        This method iterates each pixel of one video frame
        while respecting the dimensions of the printing area
    ...
    def render(self, cap, output=None, with_audio=False):
        ...
        This method read each video frame from a opec video capture
        resizing the frame and truncate the width if necessary to
        print correctly the final string builder with the method
